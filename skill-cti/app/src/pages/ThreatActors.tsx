...
function StatusBadge({ status }: { status: RansomwareStatus }) {
  ...
  return (
    <span
      className="text-[8px] font-bold px-1.5 py-[3px] rounded leading-none"
    ...
    <div className="flex flex-wrap gap-1.5">
      {coverage.map((tac) => {
        ...
        return (
          <div
            key={tac.shortname}
            className="flex items-center gap-1.5 px-2 py-[5px] rounded text-[9px] font-bold leading-none"
            style={{ background: `${color}18`, border: `1px solid ${color}35`, color }}
          >
            <span>{tac.name}</span>
            <span
              className="font-mono text-[8px] px-1 py-[1px] rounded leading-none"
...
function TechRow({ tech }: { tech: GroupTechnique }) {
  ...
  return (
    ...
    >
      <span className="text-[9px] font-bold font-mono w-[68px] flex-shrink-0 leading-none" style={{ color }}>
        {tech.attack_id}
      </span>
      ...
      {tech.is_subtechnique && (
        <span className="text-[8px] text-txt-3 font-mono flex-shrink-0">sub</span>
      )}
    </Link>
  )
}
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <p className="text-[8px] font-bold tracking-[0.15em] text-txt-3 uppercase">ATT&CK Techniques</p>
        <button
          onClick={() =>
            allExpanded
          ...
          className="text-[9px] text-txt-3 hover:text-txt-2 bg-transparent border-none cursor-pointer p-0"
        >
          ...
          return (
            <div key={tac.shortname} className="rounded-lg border border-border/60 overflow-hidden">
              ...
              >
                ...
                <span className="text-[10px] font-bold flex-1 text-left" style={{ color }}>
                  {tac.name}
                </span>
                <span className="text-[9px] font-mono text-txt-3">{tac.techniques.length}</span>
                <svg
...
function GenerateDetectionsButton({
  ...
  if (status === 'done' && job?.result_report_id) {
    return (
      <Link
        ...
        className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[9px] font-bold transition-colors"
  ...
  if (status === 'pending' || status === 'running') {
    return (
      <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[9px] font-bold"
  ...
  if (err || status === 'error') {
    return (
      <button
        onClick={launch}
        className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[9px] font-bold cursor-pointer border transition-colors hover:opacity-80"
  ...
  return (
    <button
      ...
      className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[9px] font-bold cursor-pointer border transition-colors hover:opacity-80 disabled:opacity-40 disabled:cursor-not-allowed"
  ...
  return (
    <div className="h-full overflow-y-auto">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-1.5 flex-wrap">
              <span className="text-[9px] font-bold font-mono text-purple">{group.attack_id}</span>
              <span className="text-[8px] font-bold px-1.5 py-[3px] rounded leading-none bg-purple/10 text-purple border border-purple/20">
                MITRE ATT&CK
              </span>
              <span className="text-[9px] text-txt-3 font-mono">{group.technique_count} techniques</span>
            </div>
            <h2 className="text-[20px] font-bold text-txt leading-tight mb-3">{group.name}</h2>
            {group.aliases.length > 0 && (
              <div className="flex flex-wrap gap-1.5">
                {group.aliases.map((a) => (
                  <span key={a} className="text-[9px] font-mono px-2 py-[3px] rounded-full bg-purple/10 text-purple border border-purple/20">
                    {a}
                  </span>
                ))}
              </div>
          ...
          <div className="flex-shrink-0 flex flex-col items-end gap-2">
            <a href={mitreUrl} target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-1 text-[9px] font-bold font-mono text-purple hover:underline">
              MITRE ATT&CK
      ...
      <div className="px-6 py-5 flex flex-col gap-6">
        {group.description && (
          <div>
            <p className="text-[8px] font-bold tracking-[0.15em] text-txt-3 uppercase mb-2">Overview</p>
            <p className="text-[11px] text-txt-2 leading-relaxed">{desc}</p>
            {(group.description?.length ?? 0) > 480 && (
              <button onClick={() => setShowFullDesc((v) => !v)}
                className="text-[9px] text-txt-3 hover:text-txt-2 hover:underline bg-transparent border-none cursor-pointer p-0 mt-1.5">
        ...
        {group.tactic_coverage.length > 0 && (
          <div>
            <p className="text-[8px] font-bold tracking-[0.15em] text-txt-3 uppercase mb-2">Kill-Chain Coverage</p>
  ...
  return (
    <div className="h-full overflow-y-auto">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-1.5 flex-wrap">
              ...
              <span className="text-[9px] text-txt-3 font-mono">{actor.technique_count} techniques</span>
            </div>
            <h2 className="text-[20px] font-bold text-txt leading-tight mb-3">{actor.name}</h2>
            {actor.aliases.length > 0 && (
              <div className="flex flex-wrap gap-1.5">
                {actor.aliases.map((a) => (
                  <span key={a} className="text-[9px] font-mono px-2 py-[3px] rounded-full bg-orange-500/10 text-orange-400 border border-orange-500/20">
                    {a}
                  </span>
                ))}
              </div>
      ...
      <div className="px-6 py-5 flex flex-col gap-6">
        ...
        {/* Extortion methods */}
        <div>
          <p className="text-[8px] font-bold tracking-[0.15em] text-txt-3 uppercase mb-2">Extortion Methods</p>
          <div className="flex flex-wrap gap-1.5">
            {actor.extortion_methods.map((m) => (
              <span key={m} className="text-[9px] px-2 py-[4px] rounded bg-red/10 text-red border border-red/20 font-medium">
                {m}
              </span>
            ))}
          </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {actor.target_sectors.length > 0 && (
            <div>
              <p className="text-[8px] font-bold tracking-[0.15em] text-txt-3 uppercase mb-2">Target Sectors</p>
              <div className="flex flex-col gap-1">
                {actor.target_sectors.map((s) => (
                  <span key={s} className="text-[10px] text-txt-2 flex items-center gap-1.5">
          ...
          {actor.target_regions.length > 0 && (
            <div>
              <p className="text-[8px] font-bold tracking-[0.15em] text-txt-3 uppercase mb-2">Target Regions</p>
              <div className="flex flex-col gap-1">
                {actor.target_regions.map((r) => (
                  <span key={r} className="text-[10px] text-txt-2 flex items-center gap-1.5">
                    <span className="w-1 h-1 rounded-full bg-txt-3 flex-shrink-0" />
                    {r}
                  </span>
                ))}
        ...
        {actor.notable_tools.length > 0 && (
          <div>
            <p className="text-[8px] font-bold tracking-[0.15em] text-txt-3 uppercase mb-2">Notable Tools</p>
            <div className="flex flex-wrap gap-1.5">
              {actor.notable_tools.map((t) => (
                <span key={t} className="text-[9px] font-mono px-2 py-[3px] rounded bg-bg border border-border text-txt-2">
                  {t}
                </span>
              ))}
            </div>
        ...
        {actor.notable_campaigns.length > 0 && (
          <div>
            <p className="text-[8px] font-bold tracking-[0.15em] text-txt-3 uppercase mb-2">Notable Campaigns</p>
            <div className="flex flex-col gap-1.5">
              {actor.notable_campaigns.map((c) => (
                <div key={c} className="flex items-start gap-2">
                  <span className="w-1 h-1 rounded-full bg-orange-400 flex-shrink-0 mt-[6px]" />
                  <span className="text-[10px] text-txt-2 leading-relaxed">{c}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {actor.references.length > 1 && (
          <div>
            <p className="text-[8px] font-bold tracking-[0.15em] text-txt-3 uppercase mb-2">References</p>
            <div className="flex flex-col gap-1">
              {actor.references.map((ref) => (
                <a key={ref} href={ref} target="_blank" rel="noopener noreferrer"
                  className="text-[9px] font-mono text-purple hover:underline truncate">
                  {ref}
                </a>
              ))}
            </div>
...
function MetaCell({ label, value, colSpan }: { label: string; value: string; colSpan?: boolean }) {
  return (
    <div className={colSpan ? 'col-span-2' : ''}>
      <p className="text-[8px] font-bold tracking-[0.1em] text-txt-3 uppercase mb-1">{label}</p>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-0.5">
        <span className="text-[9px] font-bold font-mono text-purple flex-shrink-0">{group.attack_id}</span>
        <span className={['text-[11px] font-bold truncate flex-1', selected ? 'text-purple' : 'text-txt'].join(' ')}>
          {group.name}
        </span>
      </div>
      {group.aliases.length > 0 && (
        <p className="text-[9px] text-txt-3 truncate pl-[2px]">
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-0.5">
        ...
        <span
          className="text-[7px] font-bold px-1 py-[2px] rounded flex-shrink-0 leading-none"
      ...
      {actor.aliases.length > 0 && (
        <p className="text-[9px] text-txt-3 truncate">
    ...
    <div className="flex gap-1 p-1 bg-bg rounded-lg border border-border">
      {tabs.map((t) => (
        <button
          ...
          className={[
            'flex-1 text-[9px] font-bold py-1.5 rounded-md transition-colors cursor-pointer border-none',
            value === t.id
    ...
    <div className="flex h-full overflow-hidden">
      ...
      <div className="w-[260px] flex-shrink-0 border-r border-border flex flex-col h-full bg-surface/40">
        <div className="p-4 border-b border-border flex-shrink-0 flex flex-col gap-2.5">
          ...
          <p className="text-[9px] text-txt-3 font-mono h-[12px]">
        ...
        <div className="flex-1 overflow-y-auto">
            ...
            <div className="flex flex-col items-center justify-center py-6 gap-2 text-txt-3 px-4">
              {isAttackLoading ? (
                <>
                  ...
                  <p className="text-[9px] text-center">Downloading ATT&CK bundle...</p>
                </>
              ) : (
                <p className="text-[9px] text-center text-txt-3">ATT&CK data not yet loaded</p>
              )}
            </div>
          )}
          ...
          {showRansom && filter === 'all' && ransomActors.length > 0 && (
            <div className="px-4 py-2 border-b border-border/40 bg-bg/40">
              <span className="text-[8px] font-bold tracking-[0.15em] text-orange-400 uppercase">
                Ransomware Groups
              </span>
            </div>
          )}
          ...
          {showApt && filter === 'all' && aptGroups.length > 0 && (
            <div className="px-4 py-2 border-b border-border/40 bg-bg/40">
              <span className="text-[8px] font-bold tracking-[0.15em] text-purple uppercase">
                MITRE ATT&CK Groups
              </span>
            </div>
          )}
