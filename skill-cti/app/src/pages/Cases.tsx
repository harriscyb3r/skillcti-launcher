...
function StatusBadge({ status }: { status: CaseStatus }) {
  ...
  return (
    <span className="text-[8px] font-bold tracking-[0.1em] px-1.5 py-[2px] rounded"
...
function NewCaseModal({ onClose, onCreate }: { onClose: () => void; onCreate: (c: Case) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        <div>
          <p className="text-[9px] font-bold tracking-[0.2em] text-txt-3 uppercase mb-1">New Investigation</p>
          <h2 className="text-[16px] font-bold text-txt">Create Case</h2>
        </div>

        <div>
          <label className="text-[10px] font-semibold tracking-[0.08em] text-txt-2 uppercase block mb-1.5">Title</label>
          <input
            ref={titleRef}
            value={title}
        ...
        <div>
          <label className="text-[10px] font-semibold tracking-[0.08em] text-txt-2 uppercase block mb-1.5">Description</label>
          <textarea
            value={description}
        ...
        <div>
          <label className="text-[10px] font-semibold tracking-[0.08em] text-txt-2 uppercase block mb-1.5">Priority</label>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <span className="text-[9px] text-txt-3 ml-auto">{timeAgo(c.updated_at)}</span>
      </div>
    </button>
  )
}
...
function ArtifactRow({ a, onDelete }: { a: CaseArtifact; onDelete: () => void }) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-2.5 border-b border-border/50 group">
      <span className="text-[8px] font-bold px-1.5 py-[2px] rounded flex-shrink-0" style={{ background: m.bg, color: m.color }}>
        {m.label}
      </span>
      <span className="flex-1 text-[11px] font-mono text-txt truncate" title={a.value}>{a.value}</span>
      {a.label && a.label !== a.value && (
        <span className="text-[10px] text-txt-3 truncate max-w-[120px]">{a.label}</span>
      )}
      <button
        onClick={onDelete}
  ...
  return (
    <div className="group relative px-4 py-3 border-b border-border/50">
      <div className="flex items-center justify-between mb-1">
        <span className="text-[9px] text-txt-3 font-mono">{new Date(n.created_at).toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
        <button
          onClick={onDelete}
          className="opacity-0 group-hover:opacity-100 transition-opacity text-txt-3 hover:text-red bg-transparent border-none cursor-pointer p-0"
          title="Delete note"
      ...
      <div className="px-6 py-4 border-b border-border flex-shrink-0">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select
            ...
            className="text-[9px] font-bold tracking-[0.1em] px-2 py-[3px] rounded border-none outline-none cursor-pointer"
          ...
          <select
            ...
            className="text-[9px] font-bold tracking-[0.1em] px-2 py-[3px] rounded bg-bg border border-border text-txt-2 outline-none cursor-pointer"
          >
          ...
          {confirmDelete ? (
            <div className="flex items-center gap-2">
              <button onClick={() => deleteCaseMut.mutate()} className="text-[9px] font-bold text-red hover:opacity-80 bg-transparent border-none cursor-pointer">Delete</button>
              <button onClick={() => setConfirmDelete(false)} className="text-[9px] text-txt-3 hover:text-txt-2 bg-transparent border-none cursor-pointer">Cancel</button>
            </div>
          ) : (
            <button
              onClick={() => setConfirmDelete(true)}
              className="text-[9px] text-txt-3 hover:text-red transition-colors bg-transparent border-none cursor-pointer"
            >
              Delete case
            </button>
          )}
      ...
      <div className="flex border-b border-border flex-shrink-0 px-6">
        {(['overview', 'artifacts', 'notes', 'timeline'] as const).map((tab) => (
          <button
            ...
            className={[
              'text-[10px] font-bold tracking-[0.08em] uppercase py-2.5 mr-5 border-b-2 transition-colors bg-transparent cursor-pointer',
              activeTab === tab
          ...
          >
            ...
            {tab === 'artifacts' && c.artifact_count > 0 && (
              <span className="ml-1.5 text-[8px] font-bold px-1 py-0.5 rounded-full bg-purple/15 text-purple leading-none">
                {c.artifact_count}
              </span>
            )}
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {activeTab === 'overview' && (
          <div className="p-6 flex flex-col gap-5">
            {/* Description */}
            <div>
              <p className="text-[9px] font-bold tracking-[0.2em] text-txt-3 uppercase mb-2">Description</p>
              <DescriptionEditor
                value={c.description}
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <div key={label} className="bg-surface border border-border rounded-lg p-3">
                  <p className="text-[8px] font-bold tracking-[0.15em] text-txt-3 uppercase mb-1">{label}</p>
            ...
            {(c.artifacts ?? []).length > 0 && (
              <div>
                <p className="text-[9px] font-bold tracking-[0.2em] text-txt-3 uppercase mb-2">Recent Artifacts</p>
                  ...
                  {(c.artifacts ?? []).slice(-3).reverse().map((a) => {
                    ...
                    return (
                      <div key={a.id} className="flex items-center gap-3 px-4 py-2.5 border-b border-border/50 last:border-0">
                        <span className="text-[8px] font-bold px-1.5 py-[2px] rounded" style={{ background: m.bg, color: m.color }}>{m.label}</span>
            ...
            {(c.notes ?? []).length > 0 && (
              <div>
                <p className="text-[9px] font-bold tracking-[0.2em] text-txt-3 uppercase mb-2">Latest Note</p>
                <div className="bg-surface border border-border rounded-lg p-4">
                  <p className="text-[9px] text-txt-3 font-mono mb-1">
            ...
            <div className="p-4 border-b border-border flex-shrink-0">
              {showArtifactForm ? (
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="px-3 py-1.5 text-[10px] font-bold rounded bg-gradient-to-br from-purple to-purple-dark text-white border-none cursor-pointer disabled:opacity-40"
                    >
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 text-[10px] font-bold text-txt-3 hover:text-txt-2 transition-colors bg-transparent border-none cursor-pointer"
                >
                    ...
                    <div key={item.id} className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          {item.kind === 'created' && <span className="text-[10px] font-bold text-txt-2">Case opened</span>}
                          {isNote && <span className="text-[10px] font-bold text-txt-2">Note added</span>}
                          {isArtifact && artifactMeta && (
                            <>
                              <span className="text-[8px] font-bold px-1.5 py-[2px] rounded" style={{ background: artifactMeta.bg, color: artifactMeta.color }}>{artifactMeta.label}</span>
                              <span className="text-[10px] font-bold text-txt-2">artifact linked</span>
                            </>
                          )}
                          <span className="text-[9px] text-txt-3 font-mono ml-auto">
  ...
  if (editing) {
    return (
      <div>
        ...
        />
        <p className="text-[9px] text-txt-3 mt-1">Click outside to save</p>
      </div>
    )
  }
      ...
      <div className="w-[300px] flex-shrink-0 border-r border-border flex flex-col h-full bg-surface/50">
        ...
        <div className="p-4 border-b border-border flex-shrink-0">
          <div className="flex items-center justify-between mb-3">
            <div>
              <p className="text-[9px] font-bold tracking-[0.2em] text-txt-3 uppercase">Investigations</p>
              <p className="text-[15px] font-bold text-txt">Cases</p>
            </div>
            <button
              onClick={() => setShowNewCase(true)}
              className="flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-bold rounded-lg bg-gradient-to-br from-purple to-purple-dark text-white border-none cursor-pointer hover:opacity-90 transition-opacity"
            >
          ...
          <div className="flex gap-1 flex-wrap">
            {([
              ...
              <button
                ...
                className={[
                  'text-[9px] font-bold tracking-[0.08em] px-2.5 py-1 rounded border transition-all cursor-pointer',
                  statusFilter === key
