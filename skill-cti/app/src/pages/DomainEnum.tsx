  ...
  return (
    <div className="bg-surface border border-border rounded-lg shadow-card p-4 flex flex-col gap-3">
      <div className="flex items-center gap-2">
        ...
        {geo.country_code && (
          <span className="text-[9px] font-bold font-mono px-1.5 py-0.5 rounded bg-bg border border-border text-txt-3">
            {geo.country_code}
          </span>
        )}
      </div>
...
function Row({ label, value, full }: { label: string; value: string; full?: boolean }) {
  return (
    <div className={full ? 'col-span-2' : ''}>
      <div className="text-[8px] font-bold tracking-[0.12em] text-txt-3 uppercase mb-0.5">{label}</div>
      ...
      {orderedTypes.map((type) => (
        <div key={type}>
          <div className="flex items-center gap-2 mb-2">
            <span className={`text-[8px] font-bold font-mono px-2 py-[3px] rounded border ${dnsTypeStyle(type)}`}>
              {type}
            </span>
            <span className="text-[9px] text-txt-3 font-mono">{grouped[type].length} record{grouped[type].length !== 1 ? 's' : ''}</span>
          </div>
          <div className="bg-surface border border-border rounded-lg shadow-card overflow-hidden">
            {grouped[type].map((r, i) => (
              ...
              >
                {r.priority != null && (
                  <span className="text-[9px] font-mono text-txt-3 w-8 flex-shrink-0 pt-px">
                    {r.priority}
                  </span>
                )}
                ...
                {r.ttl != null && (
                  <span className="text-[8px] font-mono text-txt-3 flex-shrink-0 pt-px">
                    TTL {r.ttl}s
                  </span>
                )}
              </div>
  ...
  return (
    <div className="flex flex-col gap-4">
      {rows.length > 0 && (
        <div className="bg-surface border border-border rounded-lg shadow-card overflow-hidden">
          {rows.map((row, i) => (
            ...
            >
              <span className="text-[8px] font-bold tracking-[0.12em] text-txt-3 uppercase w-20 flex-shrink-0 pt-px">{row.label}</span>
      ...
      {Array.isArray(whois.status) && whois.status.length > 0 && (
        <div>
          <div className="text-[8px] font-bold tracking-[0.15em] text-txt-3 uppercase mb-2">Status</div>
          <div className="flex flex-wrap gap-1.5">
            {whois.status.filter(Boolean).map((s) => (
              <span key={s} className="text-[9px] font-mono px-2 py-0.5 rounded bg-bg border border-border text-txt-2">
                {s}
              </span>
            ))}
          </div>
      ...
      {Array.isArray(whois.nameservers) && whois.nameservers.length > 0 && (
        <div>
          <div className="text-[8px] font-bold tracking-[0.15em] text-txt-3 uppercase mb-2">Nameservers</div>
          <div className="flex flex-wrap gap-1.5">
            {whois.nameservers.map((ns) => (
              <span key={ns} className="text-[9px] font-mono px-2 py-0.5 rounded bg-cyan/8 text-cyan border border-cyan/20">
                {ns}
              </span>
            ))}
          </div>
      ...
      {Array.isArray(whois.emails) && whois.emails.length > 0 && (
        <div>
          <div className="text-[8px] font-bold tracking-[0.15em] text-txt-3 uppercase mb-2">Contact Emails</div>
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <span className="text-[8px] font-mono text-txt-3 px-2 py-0.5 rounded bg-surface border border-border">
        ...
        <>
          ...
          {hasMore && (
            <button
              ...
              className="text-[10px] text-txt-3 hover:text-txt-2 transition-colors bg-transparent border-none cursor-pointer text-left"
            >
          ...
          entries.map((e, i) => (
            ...
            >
              ...
              <div className="flex items-center gap-2 mt-0.5">
                <span className="text-[9px] text-txt-3 font-mono">{timeAgo(e.timestamp)}</span>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        <div className="flex gap-2 mb-8">
          ...
          <button
            ...
            className="px-5 py-2.5 text-[10px] font-bold tracking-[0.1em] rounded-lg bg-gradient-to-br from-purple to-purple-dark text-white border-none cursor-pointer disabled:opacity-40 transition-opacity hover:opacity-90 flex items-center gap-2"
          >
            {loading ? (
              <>
        ...
        {hasResult && (
          <div className="flex flex-col gap-8">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <span className="text-[9px] font-mono text-txt-3 px-2 py-0.5 rounded bg-surface border border-border">
