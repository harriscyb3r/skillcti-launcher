...
import type { WatchAlert, WatchItem, WatchItemType } from '../lib/types'
...
function parseUpload(text: string, fallbackType: WatchItemType): { type: WatchItemType; value: string; label: string }[] {
  return text
    ...
    .map((value) => ({
      type: fallbackType === 'ioc' || fallbackType === 'cve' ? fallbackType : autoDetectType(value),
      value,
      label: '',
    }))
}
...
function WatchListTab() {
  ...
  const grouped: Record<WatchItemType, WatchItem[]> = { ioc: [], cve: [], vendor: [], threat_actor: [] }
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="bg-surface border border-border rounded-lg shadow-card p-4">
        ...
        <div className="flex gap-2 mb-3">
          {(['ioc', 'cve', 'vendor', 'threat_actor'] as WatchItemType[]).map((t) => {
            const m = TYPE_META[t]
            return (
              <button
                key={t}
                onClick={() => setAddType(t)}
                className="text-[9px] font-bold px-3 py-1.5 rounded border transition-colors cursor-pointer bg-transparent"
                style={
                  addType === t
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="px-4 py-2 text-[10px] font-bold tracking-[0.1em] rounded-lg border border-border text-txt-2 bg-transparent hover:border-border2 cursor-pointer transition-colors flex items-center gap-2"
          >
          ...
          <span className="text-[9px] text-txt-3 font-mono">Ctrl+Enter to add · .txt / .csv supported</span>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center gap-3 py-12 text-center">
          ...
          <div>
            ...
            <p className="text-[11px] text-txt-3 mt-1">Add IOCs, CVEs, vendor names, or threat actor names above to start tracking.</p>
          </div>
        </div>
      ) : (
        <div className="flex flex-col gap-4">
          {(['ioc', 'cve', 'vendor', 'threat_actor'] as WatchItemType[]).map((type) => {
            ...
            return (
              <div key={type}>
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-[8px] font-bold font-mono px-2 py-[3px] rounded" style={{ background: m.bg, color: m.color }}>
                    {m.label}
                  </span>
                  <span className="text-[9px] text-txt-3 font-mono">{typeItems.length} item{typeItems.length !== 1 ? 's' : ''}</span>
                </div>
                <div className="bg-surface border border-border rounded-lg shadow-card overflow-hidden">
                  <div className="grid grid-cols-[1fr_auto_auto_auto] text-[8px] font-bold tracking-[0.15em] text-txt-3 uppercase px-4 py-2 border-b border-border bg-bg">
                    <span>Value</span>
                    ...
                    >
                      <div className="min-w-0">
                        ...
                        {item.label && item.label !== item.value && (
                          <div className="text-[9px] text-txt-3 truncate">{item.label}</div>
                        )}
                      </div>
                      <div className="text-[9px] text-txt-3 font-mono text-right flex-shrink-0">
                        {item.last_checked ? timeAgo(item.last_checked) : 'Never'}
                      </div>
                      <div className="flex-shrink-0">
                        {item.last_state ? (
                          <span className="text-[8px] font-mono text-green px-1.5 py-[2px] rounded bg-green/10 border border-green/20">
                            Checked
                          </span>
                        ) : (
                          <span className="text-[8px] font-mono text-txt-3 px-1.5 py-[2px] rounded bg-surface border border-border">
                            Pending
                          </span>
                        )}
                      </div>
    ...
    <div className="flex flex-col gap-4">
      {alerts.length > 0 && (
        <div className="flex items-center gap-3">
          <button
            ...
            className="text-[9px] font-bold text-txt-3 hover:text-txt-2 bg-transparent border-none cursor-pointer disabled:opacity-40 transition-colors"
          >
            Mark all read
          </button>
          ...
          <button
            ...
            className="text-[9px] font-bold text-txt-3 hover:text-red bg-transparent border-none cursor-pointer disabled:opacity-40 transition-colors"
          >
            Clear all
          </button>
        </div>
      ...
      {alerts.length === 0 ? (
        <div className="bg-surface border border-border rounded-lg shadow-card p-8 text-center">
          ...
          <p className="text-[10px] text-txt-3 mt-1">Changes detected during monitoring will appear here.</p>
        </div>
      ) : (
        <div className="flex flex-col gap-2">
          {pg.paged.map((alert) => {
            ...
            // Prefer structured matches; fall back to parsing old bullet-text detail format.
            // Old format has no stored URL so we generate a Google search link from the title.
            ...
            // For old format (bullets embedded in detail), show only the preamble sentence
            const displayDetail =
              ...
              >
                ...
                >
                  <div className="flex items-center gap-2 flex-wrap min-w-0">
                    ...
                    <span
                      className="text-[8px] font-bold font-mono px-1.5 py-[2px] rounded flex-shrink-0"
                      style={{ background: sev.bg, color: sev.color }}
                    >
                      {sev.label}
                    </span>
                    <span
                      className="text-[8px] font-mono px-1.5 py-[2px] rounded flex-shrink-0"
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <span className="text-[9px] text-txt-3 font-mono">{timeAgo(alert.created_at)}</span>
                    <button
                ...
                <p className="text-[10px] text-txt-2 mt-1 leading-relaxed">{displayDetail}</p>
                <p className="text-[9px] text-txt-3 font-mono mt-2">{alert.item_value}</p>
                ...
                {hasMatches && (
                  <div className="mt-3 border-t border-border pt-2.5">
                    <button
                      ...
                      className="flex items-center gap-1.5 text-[9px] font-bold text-txt-3 hover:text-accent transition-colors bg-transparent border-none cursor-pointer p-0"
                    >
                      <svg
                      ...
                      <div className="mt-2 flex flex-col gap-1.5">
                        {displayMatches.map((m, i) => (
                          <a
                            ...
                            className="flex items-start gap-2 text-[10px] text-accent hover:text-txt transition-colors group"
...
export default function Watchlist() {
  ...
  return (
    <div className="p-6 overflow-y-auto h-full">
      <p className="text-[11px] font-bold tracking-[0.2em] text-txt-3 uppercase mb-3">Monitoring</p>
      <h1 className="text-[22px] font-bold text-txt tracking-tight mb-1">Watch List</h1>
      <p className="text-[13px] text-txt-2 mb-6">
        Pin IOCs, CVEs, vendors, and threat actors to monitor. The backend checks for changes on a schedule and surfaces
        alerts for verdict flips, new KEV entries, EPSS spikes, new CVEs, and threat actor activity.
      </p>
      ...
      <div className="flex items-center gap-1 mb-6 border-b border-border">
        {([
          ...
          <button
            ...
            onClick={() => setTab(id)}
            className={[
              'px-4 py-2.5 text-[10px] font-bold tracking-[0.1em] uppercase border-none bg-transparent cursor-pointer transition-colors flex items-center gap-2 -mb-px border-b-2',
              tab === id
          ...
          >
            {label}
            {id === 'alerts' && unread > 0 && (
              <span className="text-[10px] font-bold px-1.5 py-[1px] rounded-full bg-red/15 text-red border border-red/25">
                {unread}
              </span>
            )}
          </button>
      ...
      {tab === 'watchlist' ? <WatchListTab /> : <AlertsTab />}
    </div>
  )
}
