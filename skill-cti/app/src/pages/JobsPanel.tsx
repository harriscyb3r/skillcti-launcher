...
function StatusBadge({ status }: { status: Job['status'] }) {
  ...
  return (
    <span className={`text-[8px] font-bold uppercase tracking-[0.1em] px-2 py-[2px] rounded border ${cls} leading-none`}>
      {status === 'running' && (
  ...
  return (
    <div className="bg-surface border border-border rounded-lg shadow-card p-4 flex flex-col gap-3">
      ...
      <div className="flex items-start gap-3">
        <div
          className="text-[8px] font-bold tracking-[0.12em] uppercase px-2 py-[3px] rounded border flex-shrink-0 mt-[1px]"
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <span className="text-[9px] font-bold uppercase text-txt-3 font-mono">{job.format}</span>
      ...
      {(job.status === 'done' || job.status === 'error') && (
        <div className="flex gap-2 pt-1 border-t border-border">
          {job.status === 'done' && job.format === 'html' && (
            <button
              onClick={handleView}
              className="flex-1 py-1.5 text-[10px] font-semibold rounded-lg bg-purple/10 border border-purple/30 text-purple hover:bg-purple/20 transition-colors cursor-pointer"
            >
              View Report
            </button>
          )}
          ...
          <button
            ...
            className="px-3 py-1.5 text-[10px] font-semibold rounded-lg border border-border text-txt-3 hover:text-red hover:border-red/30 transition-colors cursor-pointer bg-transparent"
          >
            Remove
          </button>
        </div>
      ...
      {(job.status === 'pending' || job.status === 'running') && (
        <div className="flex justify-end pt-1 border-t border-border">
          <button
            onClick={() => onDelete(job.id)}
            className="px-3 py-1.5 text-[10px] font-semibold rounded-lg border border-border text-txt-3 hover:text-red hover:border-red/30 transition-colors cursor-pointer bg-transparent flex items-center gap-1.5"
          >
...
export default function JobsPanel() {
  ...
  return (
    <div className="flex flex-col gap-6 p-6 max-w-3xl mx-auto">
      ...
      {running.length > 0 && (
        <section>
          <h2 className="text-[10px] font-bold uppercase tracking-[0.1em] text-txt-3 mb-3">
