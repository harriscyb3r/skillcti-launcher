...
def list_reports() -> list[dict]:
    ...
    for pdf_file in reports_dir.glob("*.pdf"):
        meta_file = reports_dir / f"{pdf_file.stem}.meta.json"
        if not meta_file.exists():
            _adopt_orphan_pdf(pdf_file, reports_dir)
...
def _adopt_orphan_pdf(pdf_file: Path, reports_dir: Path) -> dict:
    from datetime import datetime, timezone
    original_stem = pdf_file.stem
    safe_stem = "".join(c if c not in "<>\"'\x00\r\n" else "_" for c in original_stem)
    if safe_stem != original_stem:
        new_path = reports_dir / f"{safe_stem}.pdf"
...
async def get_report(report_id: str):
    ...
    pdf_file = settings.reports_dir / f"{rid}.pdf"
    if meta.get("format") == "pdf" and pdf_file.exists():
...
async def get_spend_summary():
    """Total API spend across reports and activity (Threat Pulse, Spotlight, Analyst chat)."""
    from services.api_spend import read_spend_log

    # Sum report costs
    reports_total = 0.0
    for meta in list_reports():
        reports_total += meta.get("cost_usd") or 0.0

    # Sum activity costs by source
    activity_by_source: dict[str, float] = {}
    activity_total = 0.0
    for entry in read_spend_log():
        src = entry.get("source", "unknown")
        cost = entry.get("cost_usd") or 0.0
        activity_by_source[src] = activity_by_source.get(src, 0.0) + cost
        activity_total += cost

    return {
        "reports_total": round(reports_total, 4),
        "activity_total": round(activity_total, 4),
        "grand_total": round(reports_total + activity_total, 4),
        "activity_by_source": {k: round(v, 4) for k, v in activity_by_source.items()},
    }
