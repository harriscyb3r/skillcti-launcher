...
async def _sse_stream(body: bytes, source: str = "analyst-chat") -> AsyncGenerator[str, None]:
    """Forward Anthropic SSE stream to the browser as SSE."""
    target = f"{ANTHROPIC_API}/v1/messages"
    headers = _make_headers()

    try:
        model: str = json.loads(body).get("model", "claude-sonnet-4-6")
    except Exception:
    ...
    if input_tokens or output_tokens:
        ...
        record_spend(source, model, input_tokens, output_tokens, cache_read, cache_write)
...
async def stream_chat(request: Request):
    ...
    return StreamingResponse(
        _sse_stream(raw, source="analyst-chat"),
...
async def stream_anthropic(request: Request):
    ...
    return StreamingResponse(
        _sse_stream(raw, source="skill-proxy"),
        media_type="text/event-stream",
        headers={
