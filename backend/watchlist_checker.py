...
# ── Credential / stealer log checker ──────────────────────────────────────────
...
async def _check_credential(client: httpx.AsyncClient, item: dict) -> list[dict[str, Any]]:
    """Monitor a domain or email for new HudsonRock infostealer hits."""
    alerts: list[dict[str, Any]] = []
    query = item["value"].strip()
    last  = item.get("last_state") or {}
    by_email = _is_email(query)
    ...
    try:
        ...
        if isinstance(data, dict):
            new_total = int(data.get("total") or 0)
        elif isinstance(data, list):
            new_total = len(data)
        else:
            return alerts

        old_total = last.get("hr_total")

        if old_total is None:
            # First check — baseline only, no alert
            item["last_state"] = {**(item.get("last_state") or {}), "hr_total": new_total}
            return alerts

        delta = new_total - old_total
        if delta > 0:
            severity = "critical" if delta >= 50 else "high" if delta >= 10 else "medium"
            alerts.append({
                "alert_type": "new_stealer_hit",
                "severity":   severity,
                "title":      f"{query} — {delta:,} new stealer log hit{'s' if delta != 1 else ''}",
                "detail":     (
                    f"HudsonRock Cavalier detected {delta:,} new infostealer compromise record{'s' if delta != 1 else ''} "
                    f"for {query} (previously {old_total:,}, now {new_total:,} total). "
                    f"Credentials from affected machines may be actively circulating."
                ),
            })
        ...
        print(f"    Credential check failed for {query}: {e}")
