...
const DFIR_SKILLS = [
  ...
  },
  {
    ...
    badge:'PIR', badgeColor:'#8b5cf6',
    category:'dfir',
    description:'Takes a cyber security incident report (URL, PDF, or pasted text) and produces a structured Post-Incident Review. Covers incident timeline reconstruction, root cause analysis (5 Whys), MITRE ATT&CK TTP mapping, detection gap analysis, containment and eradication effectiveness, lessons learned, control improvement recommendations mapped to NIST CSF / Essential Eight / ISO 27001, DRAFT Sigma/KQL detection stubs for gaps identified, and an action register with owners and due dates.',
    inputs:[
      {id:'input',label:'Incident report — URL · pasted text · file content',type:'textarea',required:true,placeholder:'https://company.com/incident-report\nor paste the incident report / after-action review text directly'},
    ...
    buildMsg:v=>`${v.incident?'Incident: '+v.incident+'\n\n':''}${v.severity?'Severity: '+v.severity+'\n\n':''}Regulatory region: ${v.region||'AU'}\n\nIncident report:\n${v.input.trim()}`,
    needsSearch:true, maxTokens:16000,
    skillPath:'dfir-post-incident-review'
  },
  {
