...
export const api = {
  ...
  spendSummary(): Promise<{
    reports_total: number
    activity_total: number
    grand_total: number
    activity_by_source: Record<string, number>
  }> {
    return request('/api/spend-summary')
  },
  ...
  getIntegrations(): Promise<Record<string, boolean>> {
    return request<Record<string, boolean>>('/api/integrations')
  },
  ...
  iocSummary(enrich: EnrichResult, malware_intel?: MalwareIntelResult | null): Promise<{ summary: string }> {
    return request<{ summary: string }>('/api/ioc-summary', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ enrich, malware_intel: malware_intel ?? null }),
    })
  },
  ...
  identityExposure(query: string): Promise<IdentityExposureResult> {
    return request<IdentityExposureResult>('/api/identity-exposure', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  // PIR
  listPirs(params?: { status?: string; priority?: string; category?: string }): Promise<{ pirs: Pir[] }> {
    const qs = new URLSearchParams()
    if (params?.status) qs.set('status', params.status)
    if (params?.priority) qs.set('priority', params.priority)
    if (params?.category) qs.set('category', params.category)
    const q = qs.toString()
    return request<{ pirs: Pir[] }>(`/api/pir${q ? '?' + q : ''}`)
  },

  createPir(data: CreatePirRequest): Promise<Pir> {
    return request<Pir>('/api/pir', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data),
    })
  ...
  getPir(id: string): Promise<Pir> {
    return request<Pir>(`/api/pir/${id}`)
  },

  updatePir(id: string, updates: UpdatePirRequest): Promise<Pir> {
    return request<Pir>(`/api/pir/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(updates),
  ...
  deletePir(id: string): Promise<{ ok: boolean }> {
    return request<{ ok: boolean }>(`/api/pir/${id}`, { method: 'DELETE' })
  },

  addPirFinding(pirId: string, content: string, source?: string): Promise<PirFinding> {
    return request<PirFinding>(`/api/pir/${pirId}/findings`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  deletePirFinding(pirId: string, findingId: string): Promise<{ ok: boolean }> {
    return request<{ ok: boolean }>(`/api/pir/${pirId}/findings/${findingId}`, { method: 'DELETE' })
  },

  // Abuse.ch
  getMalwareBazaar(limit = 25, force = false): Promise<{ status: string; samples: any[]; detail?: string }> {
    return request(`/abuse/malwarebazaar?limit=${limit}${force ? '&force=true' : ''}`)
  },
  getUrlHaus(limit = 25, force = false): Promise<{ status: string; urls: any[]; detail?: string }> {
    return request(`/abuse/urlhaus?limit=${limit}${force ? '&force=true' : ''}`)
  },
  getThreatFoxFeed(days = 1, limit = 30, force = false): Promise<{ status: string; iocs: any[]; detail?: string }> {
    return request(`/abuse/threatfox?days=${days}&limit=${limit}${force ? '&force=true' : ''}`)
  },

  // OTX
  getOtxStatus(): Promise<OtxStatusResponse> {
    return request<OtxStatusResponse>('/otx/status')
  },
  getOtxFeed(limit = 100, force = false): Promise<OtxFeedResponse> {
    return request<OtxFeedResponse>(`/otx/feed?limit=${limit}${force ? '&force=true' : ''}`)
  },
  searchOtx(ioc: string): Promise<OtxSearchResponse> {
    return request<OtxSearchResponse>(`/otx/search?ioc=${encodeURIComponent(ioc)}`)
  },

  // MISP
  getMispStatus(): Promise<MispStatus> {
    return request<MispStatus>('/misp/status')
  },
  getMispEvents(limit = 50, page = 1): Promise<MispEventsResponse> {
    return request<MispEventsResponse>(`/misp/events?limit=${limit}&page=${page}`)
  },
  getMispEvent(id: string): Promise<MispEventDetailResponse> {
    return request<MispEventDetailResponse>(`/misp/event/${encodeURIComponent(id)}`)
  },
  searchMisp(q: string, limit = 20): Promise<MispSearchResponse> {
    return request<MispSearchResponse>(`/misp/search?q=${encodeURIComponent(q)}&limit=${limit}`)
  },
  getMispContext(skillType = 'operational', query = '', days = 30): Promise<MispContextResponse> {
    const params = new URLSearchParams({ skill_type: skillType, days: String(days) })
    if (query) params.set('query', query)
    return request<MispContextResponse>(`/misp/context?${params}`)
  },

  // Clients
  listClients(params?: { status?: string; q?: string }): Promise<{ clients: Client[] }> {
    const qs = new URLSearchParams()
    if (params?.status) qs.set('status', params.status)
    if (params?.q) qs.set('q', params.q)
    const q = qs.toString()
    return request<{ clients: Client[] }>(`/api/clients${q ? '?' + q : ''}`)
  },
  ...
  updateClient(id: string, updates: UpdateClientRequest): Promise<Client> {
    return request<Client>(`/api/clients/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(updates),
