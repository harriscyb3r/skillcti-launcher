...
import type { ArtifactType, LibraryItem, LibraryItemType, Verdict, UpdateLibraryItem } from '../lib/types'
...
function TypeBadge({ type }: { type: string }) {
  ...
  return (
    <span className="text-[11px] font-bold px-1.5 py-[2px] rounded" style={{ background: s.bg, color: s.text }}>
      {type.toUpperCase()}
    </span>
  )
}
...
function TlpBadge({ tlp }: { tlp: string }) {
  ...
  return (
    <span className="text-[10px] font-bold px-1.5 py-[2px] rounded font-mono" style={{ color, background: `${color}20`, border: `1px solid ${color}40` }}>
      {tlp}
    </span>
  )
}
...
function TagPill({ tag, onRemove }: { tag: string; onRemove?: () => void }) {
  ...
  return (
    <span
      className="inline-flex items-center gap-1 text-[11px] font-bold px-2 py-[2px] rounded leading-none"
  ...
  return (
    ...
    >
      <div className="flex items-start gap-2 mb-1">
        ...
        {vs && (
          <span className="text-[11px] font-bold px-1.5 py-[2px] rounded" style={{ background: vs.bg, color: vs.text }}>
            {vs.label}
          </span>
        )}
      </div>
      ...
      {item.summary && (
        <p className="text-[11px] text-txt-3 truncate leading-snug">{item.summary}</p>
      )}
      <div className="flex items-center gap-2 mt-1.5 flex-wrap">
        {item.tags.slice(0, 3).map((t) => (
          <span key={t} className="text-[11px] font-bold" style={{ color: tagColor(t) }}>#{t}</span>
        ))}
        {item.tags.length > 3 && <span className="text-[11px] text-txt-3">+{item.tags.length - 3}</span>}
        <span className="text-[11px] text-txt-3 ml-auto">{timeAgo(item.updated_at)}</span>
      </div>
    </button>
  )
}
...
function NewNoteForm({ onSave, onCancel }: { onSave: (item: LibraryItem) => void; onCancel: () => void }) {
  ...
  return (
    <div className="flex flex-col gap-4 h-full p-6 overflow-y-auto">
      ...
      <div>
        <label className="text-[11px] font-bold tracking-[0.15em] text-txt-3 uppercase block mb-1">Title</label>
        <input
          ref={titleRef}
          value={title}
      ...
      <div>
        <label className="text-[11px] font-bold tracking-[0.15em] text-txt-3 uppercase block mb-1">Notes</label>
        <textarea
          value={notes}
      ...
      <div>
        <label className="text-[11px] font-bold tracking-[0.15em] text-txt-3 uppercase block mb-1">Tags</label>
        ...
        <div className="flex gap-2">
          ...
          />
          <button onClick={addTag} className="px-3 py-1.5 text-[11px] font-bold rounded-lg border border-border text-txt-3 bg-transparent hover:border-purple cursor-pointer transition-colors">Add</button>
        </div>
      </div>

      <div>
        <label className="text-[11px] font-bold tracking-[0.15em] text-txt-3 uppercase block mb-1">TLP</label>
        <div className="flex gap-2">
          {TLP_OPTIONS.map((t) => (
            <button
              key={t}
              onClick={() => setTlp(t)}
              className={['px-2 py-1 text-[11px] font-bold rounded border transition-all cursor-pointer', tlp === t ? 'opacity-100' : 'opacity-40'].join(' ')}
...
function libraryArtifactType(t: LibraryItemType): ArtifactType {
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-border flex-shrink-0">
        <div className="flex items-start gap-2 mb-2 flex-wrap">
          ...
          <TlpBadge tlp={tlp} />
          {item.source && (
            <span className="text-[10px] font-mono text-txt-3">via {item.source}</span>
          )}
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 flex flex-col gap-4">
        ...
        {contentKeys.length > 0 && (
          <div>
            <p className="text-[11px] font-bold tracking-[0.15em] text-txt-3 uppercase mb-2">Enrichment Data</p>
            <div className="bg-bg rounded-lg border border-border p-3 max-h-[180px] overflow-y-auto">
              <pre className="text-[10px] font-mono text-txt-2 whitespace-pre-wrap break-all leading-relaxed">
        ...
        {/* Notes */}
        <div>
          <p className="text-[11px] font-bold tracking-[0.15em] text-txt-3 uppercase mb-2">Analyst Notes</p>
          <textarea
            value={notes}
        ...
        {/* Tags */}
        <div>
          <p className="text-[11px] font-bold tracking-[0.15em] text-txt-3 uppercase mb-2">Tags</p>
          <div className="flex flex-wrap gap-1.5 mb-2">
        ...
        {/* TLP */}
        <div>
          <p className="text-[11px] font-bold tracking-[0.15em] text-txt-3 uppercase mb-2">Traffic Light Protocol</p>
          <div className="flex gap-2">
            {TLP_OPTIONS.map((t) => (
              <button
                key={t}
                onClick={() => { setTlp(t); setDirty(true) }}
                className={['px-2 py-1 text-[11px] font-bold rounded border transition-all cursor-pointer', tlp === t ? 'opacity-100' : 'opacity-40 hover:opacity-70'].join(' ')}
                style={{ color: TLP_COLOR[t], borderColor: TLP_COLOR[t], background: tlp === t ? `${TLP_COLOR[t]}20` : 'transparent' }}
              >
                {t}
              </button>
      ...
      {/* Footer actions */}
      <div className="px-6 py-3 border-t border-border flex items-center gap-3 flex-shrink-0 flex-wrap">
        <AddToCaseButton
          artifactType={libraryArtifactType(item.type)}
          value={item.title}
          label={item.title}
          variant="full"
        />
        ...
        <div className="ml-auto">
          {!deleteConfirm ? (
            <button
              ...
              className="text-[11px] text-txt-3 hover:text-red bg-transparent border-none cursor-pointer transition-colors"
            >
              Delete
            </button>
          ) : (
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="text-[11px] font-bold text-red hover:underline bg-transparent border-none cursor-pointer"
              >
        ...
        <div className="p-4 border-b border-border flex-shrink-0">
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-bold tracking-[0.1em] rounded-lg bg-gradient-to-br from-purple to-purple-dark text-white border-none cursor-pointer transition-opacity hover:opacity-90"
            >
          ...
          <div className="relative mb-3">
            ...
            <input
              ...
              className="w-full bg-bg border border-border rounded-lg text-txt text-[11px] pl-8 pr-3 py-2 outline-none focus:border-purple transition-colors"
            />
          </div>
          ...
          <div className="flex gap-1 flex-wrap">
            {TYPE_TABS.map((tab) => (
              <button
                ...
                className={[
                  'px-2.5 py-1 text-[11px] font-bold tracking-[0.1em] uppercase rounded-full border transition-all cursor-pointer',
          ...
          <div className="px-4 py-2.5 border-b border-border/50 flex-shrink-0 flex flex-wrap gap-1.5">
            {tags.map((t) => (
              <button
                ...
                className="text-[11px] font-bold bg-transparent border-none cursor-pointer p-0 leading-none transition-opacity"
            ...
            {tagFilter && (
              <button onClick={() => setTagFilter('')} className="text-[11px] text-txt-3 bg-transparent border-none cursor-pointer ml-1">✕ clear</button>
            )}
          </div>
        )}
