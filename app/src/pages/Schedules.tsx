...
function Badge({ text, color }: { text: string; color: string }) {
  return (
    <span
      className="inline-block text-[11px] font-bold tracking-[0.08em] uppercase px-1.5 py-0.5 rounded border leading-none flex-shrink-0"
...
function ScheduleRow({
  ...
  return (
    ...
    >
      ...
      <p className="text-[11px] text-txt-3 ml-3.5">{frequencyLine(s)}</p>
      <p className={`text-[11px] ml-3.5 mt-0.5 ${overdue && s.enabled ? 'text-amber-400' : 'text-txt-3'}`}>
...
function CreateForm({ onDone }: { onDone: () => void }) {
  ...
  const labelCls = 'block text-[11px] font-bold tracking-[0.08em] text-txt-3 uppercase mb-1'

  return (
    <div className="p-5 flex flex-col gap-4">
      <div>
        <p className="text-[11px] font-bold tracking-[0.2em] text-txt-3 uppercase mb-1">New Schedule</p>
      ...
      <div>
        ...
        {selectedSkill && (
          <p className="text-[11px] text-txt-3 mt-1">{selectedSkill.tagline}</p>
        )}
      </div>
        ...
        <div className="flex gap-2">
          {(['daily', 'weekly', 'monthly'] as ScheduleFrequency[]).map((f) => (
            <button
              ...
              className={[
                'flex-1 py-1.5 text-[11px] font-bold tracking-[0.08em] uppercase rounded border transition-colors',
                frequency === f
    ...
    <div className="p-5 flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-surface rounded-md p-3 border border-border">
          <p className="text-[11px] font-bold tracking-[0.12em] text-txt-3 uppercase mb-1">Next Run</p>
        ...
        <div className="bg-surface rounded-md p-3 border border-border">
          <p className="text-[11px] font-bold tracking-[0.12em] text-txt-3 uppercase mb-1">Last Run</p>
          {s.last_run ? (
            <>
      ...
      <div className="flex gap-2 flex-wrap">
        <span className="text-[11px] px-2 py-0.5 rounded border border-border text-txt-3">
      ...
      {!editing ? (
        <div>
          ...
          <button
            ...
            className="mt-2 text-[11px] font-bold tracking-[0.08em] uppercase text-txt-3 hover:text-txt-2 transition-colors"
          >
            Edit schedule
          </button>
        </div>
        ...
        <div className="flex flex-col gap-3 border border-border rounded-md p-3 bg-bg/40">
          ...
          <div className="flex gap-2">
            <button
              onClick={saveEdits}
              className="flex-1 py-1.5 text-[11px] font-bold tracking-[0.08em] uppercase bg-purple/80 hover:bg-purple text-white rounded transition-colors"
            >
              Save changes
            </button>
            <button
              onClick={() => setEditing(false)}
              className="px-3 py-1.5 text-[11px] font-bold tracking-[0.08em] uppercase border border-border text-txt-3 rounded hover:text-txt-2 transition-colors"
            >
              Cancel
            </button>
          </div>
      ...
      <div className="flex gap-2 pt-1">
        <button
          onClick={() => trigger()}
          disabled={triggering}
          className="flex-1 py-2 text-[11px] font-bold tracking-[0.08em] uppercase border border-border text-txt-2 hover:border-border2 hover:text-txt rounded transition-colors disabled:opacity-50"
        >
        ...
        {!confirmDelete ? (
          <button
            ...
            className="px-4 py-2 text-[11px] font-bold tracking-[0.08em] uppercase border border-border text-txt-3 hover:border-red-500/40 hover:text-red-400 rounded transition-colors"
          >
            Delete
          </button>
        ) : (
          <button
            onClick={() => doDelete()}
            className="px-4 py-2 text-[11px] font-bold tracking-[0.08em] uppercase bg-red-500/10 border border-red-500/40 text-red-400 rounded transition-colors"
          >
            Confirm delete
          </button>
        )}
    ...
    <div className="flex flex-1 h-full overflow-hidden">
      ...
      <div className="w-[280px] flex-shrink-0 border-r border-border flex flex-col h-full">
        <div className="p-3 border-b border-border flex items-center justify-between gap-2 flex-shrink-0">
          <div>
            <p className="text-[11px] font-bold tracking-[0.2em] text-txt-3 uppercase">Schedules</p>
            <p className="text-[13px] font-bold text-txt">{schedules.length} configured</p>
          </div>
          <button
            onClick={openCreate}
            className="px-3 py-1.5 text-[11px] font-bold tracking-[0.08em] uppercase bg-purple/80 hover:bg-purple text-white rounded transition-colors flex-shrink-0"
          >
            + New
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {!isLoading && schedules.length === 0 && (
            <div className="p-5 text-center">
              ...
              <button
                onClick={openCreate}
                className="mt-3 px-4 py-2 text-[11px] font-bold tracking-[0.08em] uppercase bg-purple/80 hover:bg-purple text-white rounded transition-colors"
              >
