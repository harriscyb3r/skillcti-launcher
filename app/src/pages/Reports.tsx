    ...
    <div className="fixed inset-0 bg-black z-50 flex flex-col">
      <div className="flex items-center justify-between px-[18px] py-[10px] bg-surface border-b border-border flex-shrink-0">
        <div className="flex items-center gap-3">
          {data?.meta && (
            <span
              className="text-[11px] font-bold tracking-[0.14em] px-2 py-[3px] rounded"
        ...
        <div className="flex items-center gap-2">
          {data?.meta?.format === 'pdf' ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="px-3 py-[6px] text-[11px] font-bold tracking-[0.08em] rounded-md bg-gradient-to-br from-purple to-purple-dark text-white"
            >
              Open PDF
            </a>
          ) : null}
          <button
            onClick={onClose}
            className="px-3 py-[6px] text-[11px] font-bold tracking-[0.08em] rounded-md border border-border2 text-txt-2 hover:opacity-80 transition-opacity bg-transparent"
          >
            Close
          </button>
        </div>
      ...
      {data && !data.html && data.meta?.format === 'pdf' && (
        <iframe
          src={`/reports/${reportId}/pdf`}
          className="flex-1 border-none w-full"
          title={data?.meta?.title ?? 'PDF Report'}
        />
      )}
      {data && !data.html && data.meta?.format !== 'pdf' && (
...
function ReportRowSkeleton() {
  return (
    <div className="bg-surface border border-border rounded-lg shadow-card p-4 flex items-center justify-between gap-4 animate-pulse">
      <div className="flex items-center gap-3 min-w-0 flex-1">
        <div className="w-14 h-5 bg-border rounded flex-shrink-0" />
        <div className="min-w-0 flex-1">
          <div className="h-3 bg-border rounded w-2/3 mb-2" />
          <div className="h-2.5 bg-border/60 rounded w-2/5" />
        </div>
      </div>
      <div className="h-2.5 bg-border/60 rounded w-20 flex-shrink-0" />
    </div>
  )
}
    ...
    <div className="bg-surface border border-border rounded-lg shadow-card p-4 flex items-center justify-between gap-4 hover:border-border2 transition-colors">
      ...
      >
        ...
        <div className="min-w-0">
          ...
          <div className="text-[10px] text-txt-3 mt-0.5 flex items-center gap-1.5 flex-wrap font-mono">
            <span className="font-sans">{report.skillName}</span>
            <span className="opacity-40">·</span>
            <span className="font-sans uppercase tracking-[0.06em] text-[11px] opacity-60">{report.format ?? 'html'}</span>
            <span className="opacity-40">·</span>
            <span>{formatBytes(report.bytes)}</span>
            {fmtTokens((report.input_tokens ?? 0) + (report.output_tokens ?? 0)) && (
              <>
                <span className="opacity-40">·</span>
                <span title={`Input: ${(report.input_tokens ?? 0).toLocaleString()} · Output: ${(report.output_tokens ?? 0).toLocaleString()}${(report.cache_read_tokens ?? 0) > 0 ? ` · Cache read: ${report.cache_read_tokens!.toLocaleString()}` : ''}`}>
                  {fmtTokens((report.input_tokens ?? 0) + (report.output_tokens ?? 0))}
                </span>
              </>
            )}
            {fmtCost(report.cost_usd ?? 0) && (
              <>
                <span className="opacity-40">·</span>
                <span className="text-green/70">{fmtCost(report.cost_usd ?? 0)}</span>
              </>
            )}
          </div>
        </div>
      ...
      <div className="flex items-center gap-3 flex-shrink-0">
        <span className="text-[11px] text-txt-3">{formatDate(report.timestamp)}</span>
        {confirming ? (
          <div className="flex items-center gap-2">
            <button
              onClick={() => onDelete(report.id)}
              className="text-[11px] font-bold text-red hover:opacity-80 transition-opacity bg-transparent border-none cursor-pointer"
            >
              Confirm
            </button>
            <button
              onClick={() => setConfirming(false)}
              className="text-[11px] text-txt-3 hover:opacity-80 transition-opacity bg-transparent border-none cursor-pointer"
            >
              Cancel
            </button>
          </div>
    ...
    <>
      ...
      <div className="p-6">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="text-[11px] text-txt-3">{filtered.length} / {reports.length} reports</div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4">
          ...
          <div className="flex items-center rounded-md border border-border overflow-hidden">
            <button
              ...
              className={[
                'px-3 py-[7px] text-[11px] font-bold tracking-[0.08em] border-none cursor-pointer transition-colors',
                sort === 'newest'
