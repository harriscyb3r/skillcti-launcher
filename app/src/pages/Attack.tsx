...
function TacticBadge({ shortname, name }: { shortname: string; name?: string }) {
  ...
  return (
    <span
      className="text-[11px] font-bold px-1.5 py-[2px] rounded leading-none"
...
function TechniqueRow({
  ...
  return (
    ...
    >
      <span
        className="text-[10px] font-bold font-mono flex-shrink-0 w-[58px]"
        style={{ color }}
      >
        {technique.attack_id}
      </span>
      ...
      {technique.platforms.length > 0 && (
        <span className="text-[10px] text-txt-3 font-mono flex-shrink-0 hidden sm:block">
      ...
      {t && (
        <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-4">
          ...
          {t.platforms.length > 0 && (
            <div>
              <p className="text-[11px] font-bold tracking-[0.15em] text-txt-3 uppercase mb-2">Platforms</p>
          ...
          {t.description && (
            <div>
              <p className="text-[11px] font-bold tracking-[0.15em] text-txt-3 uppercase mb-2">Description</p>
              ...
              {t.description.length > 500 && (
                <button
                  ...
                  className="text-[11px] text-txt-3 hover:text-txt-2 hover:underline bg-transparent border-none cursor-pointer p-0 mt-1"
                >
              ...
              <div className="flex flex-col gap-1">
                {t.subtechniques.map((sub) => (
                  <div key={sub.id} className="flex items-center gap-2 py-1 border-b border-border/30">
                    <span className="text-[10px] font-bold font-mono text-purple/80 w-[68px] flex-shrink-0">
                      {sub.attack_id}
                    </span>
                    <span className="text-[11px] text-txt">{sub.name}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {t.groups.map((g) => (
                  <span
                    key={g.attack_id}
                    className="text-[10px] font-bold font-mono px-2 py-[3px] rounded bg-purple/10 text-purple border border-purple/20"
          ...
          {t.data_sources.length > 0 && (
            <div>
              <p className="text-[11px] font-bold tracking-[0.15em] text-txt-3 uppercase mb-2">Data Sources</p>
              <div className="flex flex-wrap gap-1.5">
                {t.data_sources.map((ds) => (
                  <span key={ds} className="text-[10px] font-mono text-txt-3 px-1.5 py-[2px] rounded bg-bg border border-border">
                    {ds}
                  </span>
                ))}
              </div>
          ...
          {t.detection && (
            <div>
              <p className="text-[11px] font-bold tracking-[0.15em] text-txt-3 uppercase mb-2">Detection</p>
              <p className="text-[11px] text-txt-2 leading-relaxed">
          ...
          {mitreUrl && (
            <a
              ...
              className="flex items-center gap-1.5 text-[10px] font-bold font-mono text-purple hover:underline w-fit"
            >
      ...
      <div className="grid grid-cols-2 gap-3 max-w-3xl lg:grid-cols-3 xl:grid-cols-4">
        {tactics.map((tac) => {
          ...
          return (
            ...
            >
              <div className="text-[10px] font-bold font-mono mb-1.5" style={{ color }}>
                {tac.attack_id}
              </div>
              ...
              <div className="text-[11px] text-txt-3 uppercase tracking-wide">techniques</div>
            </button>
          )
        })}
      </div>
          ...
          {tactics.map((tac) => {
            ...
            return (
              ...
              >
                ...
                <span className={['text-[11px] font-bold flex-1 text-left', active ? 'text-txt' : 'text-txt-3'].join(' ')}>
                  {tac.name}
                </span>
                <span className="text-[10px] font-mono flex-shrink-0" style={{ color: active ? color : '#6b7280' }}>
                  {tac.technique_count}
                </span>
              </button>
            )
