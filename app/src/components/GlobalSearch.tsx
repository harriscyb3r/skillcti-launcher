...
export default function GlobalSearch({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div
      className="fixed inset-0 z-50 bg-black/70 backdrop-blur-md pt-[14vh] animate-fadeIn"
      onClick={onClose}
    >
      <div
        className="mx-auto max-w-xl bg-surface/[0.88] backdrop-blur-xl border border-white/[0.08] rounded-xl shadow-[0_24px_64px_rgba(0,0,0,0.7)] overflow-hidden animate-paletteIn"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Input */}
        <div className="flex items-center gap-3 px-4 py-3.5 border-b border-white/[0.07]">
          <Search size={15} strokeWidth={1.75} className="text-txt-3 flex-shrink-0" />
          <input
            ref={inputRef}
            value={query}
              ...
              return (
                ...
                >
                  ...
                  <span className="flex-1 min-w-0">
                    ...
                    {r.secondary && (
                      <span className="text-[11px] text-txt-3 truncate block">{r.secondary}</span>
                    )}
                  </span>
                  {r.kind === 'news' && (
        ...
        {!query && (
          <div className="px-4 py-5 text-[11px] text-txt-3 text-center">
            Search across reports, watchlist, alerts, and news
          </div>
        )}

        {/* Footer */}
        <div className="px-4 py-2 border-t border-white/[0.07] flex items-center gap-3 text-[11px] text-txt-3">
          {[['↑↓', 'navigate'], ['↵', 'open'], ['esc', 'close']].map(([key, label]) => (
            <span key={key} className="flex items-center gap-1.5">
              <kbd className="font-mono text-[10px] px-1.5 py-0.5 rounded border border-white/[0.10] bg-white/[0.05] leading-none">{key}</kbd>
              {label}
            </span>
          ))}
        </div>
