...
function SourceBadge({ category, feed }: { category: string; feed: string }) {
  ...
  return (
    <span
      className="text-[11px] font-bold px-1 py-[1px] rounded flex-shrink-0 leading-none"
  ...
  return (
    <div className={['flex items-center gap-2 px-3 py-2 hover:bg-bg/60 transition-colors rounded', !article.read ? 'bg-purple/[0.02]' : ''].join(' ')}>
      ...
      <a
        href={article.url}
        onClick={handleOpen}
        className="flex-1 min-w-0 text-[11px] text-txt font-bold truncate leading-snug"
      >
        {article.title}
      </a>
  ...
  return (
    <div className="mb-6">
      <div className="flex items-center gap-3 mb-3">
        ...
        {unread > 0 && (
          <span className="text-[11px] font-bold px-1.5 py-0.5 rounded-full bg-purple/15 text-purple leading-none">
            {unread} unread
          </span>
        )}
        <button
          onClick={() => navigate('/feeds')}
          className="ml-auto text-[11px] font-bold text-txt-3 hover:text-txt-2 transition-colors bg-transparent border-none cursor-pointer"
        >
          View all →
        </button>
      </div>
