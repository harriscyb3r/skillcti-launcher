...
Save as `cti-report-<region-slug>-tactical-<YYYY-MM>.html` using Write (where `<region-slug>` is `global` by default). Confirm path in one concluding sentence. Do NOT echo the HTML to chat — the file is the deliverable.
