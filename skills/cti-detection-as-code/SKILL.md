...
Save as `cti-detections-<source-slug>-<YYYY-MM-DD>.html` using Write. Confirm path in one concluding sentence. Do NOT echo the HTML to chat — the file is the deliverable.
...
- **Print / Save as PDF** button, fixed top-right, calls `window.print()`. `@media print`: dark text on white, hide nav, presentation toggle, and print button; `break-inside: avoid` on tables and SVGs; page-break per detection card.
